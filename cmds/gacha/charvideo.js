...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      ...
      if (!tag) return m.reply(`⌬ El personaje ${character.name} no tiene un tag válido para buscar videos.`);
      const mediaList = await buscarVideoDelirius(tag);
      if (!mediaList.length) return m.reply(`⌬ No se encontraron videos para ${character.name}.`);
