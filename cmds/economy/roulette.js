export default {
  ...
  run: async (client, m, args, usedPrefix) => {
    ...
    if (resultColor === color) {
      ...
      user.coins += reward
      await client.sendMessage(chatId, { text: `「✿」 La ruleta salió en *${resultColor}* y has ganado *S/${reward.toLocaleString()} ${currency}*.`, mentions: [senderId] }, { quoted: m })
    } else {
      user.coins -= amount
      await client.sendMessage(chatId, { text: `「✿」 La ruleta salió en *${resultColor}* y has perdido *S/${amount.toLocaleString()} ${currency}*.`, mentions: [senderId] }, { quoted: m })
    }
  },
}