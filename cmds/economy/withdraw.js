export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    if (args[0].toLowerCase() === 'all') {
      ...
      return m.reply(`✎ Has retirado *S/${amount.toLocaleString()} ${currency}* del banco, ahora podras usarlo pero tambien podran robartelo.`)
    }
    const count = parseInt(args[0])
    if (isNaN(count) || count < 1) return m.reply(`《✧》 Debes retirar una cantidad válida.\n > Ejemplo 1 » *${usedPrefix + command} S/25000*\n> Ejemplo 2 » *${usedPrefix + command} all*`)
    if ((user.bank || 0) < count)
      return m.reply(`《✧》 No tienes suficientes *${currency}* en tu banco para retirar esa cantidad.\n> Solo tienes *S/${user.bank.toLocaleString()} ${currency}* en tu cuenta.`)
    user.bank -= count
    user.coins = (user.coins || 0) + count
    await m.reply(`✎ Has retirado *S/${count.toLocaleString()} ${currency}* del banco, ahora podras usarlo pero tambien podran robartelo.`)
  },
};