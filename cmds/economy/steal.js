...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    if (chance < 0.3) {
      ...
      return client.reply(m.chat, `⌬ El robo salió mal y perdiste *S/${loss.toLocaleString()} ${currency}*.`, m)
    }
    ...
    target.coins -= rob
    user.laststeal = Date.now() + 3600000
    client.reply(m.chat, `❀ Le robaste *S/${rob.toLocaleString()} ${currency}* a *${name}*`, m, { mentions: [who] })
  }
}
