...
client.sendImageAsSticker = async (jid, path, quoted, options = {}) => {
  ...
  if (options && (options.packname || options.author)) {
    stickerData = await writeExifImg(buff, options)
  } else {
    stickerData = await imageToWebp(buff)
  }

  if (!stickerData) throw new Error('No se pudo generar el sticker')

  await client.sendMessage(
    jid,
    Buffer.isBuffer(stickerData)
      ? { sticker: stickerData }
      : { sticker: { url: stickerData } },
    { quoted }
  )
...
client.sendVideoAsSticker = async (jid, path, quoted, options = {}) => {
  ...
  let stickerData
  if (options && (options.packname || options.author)) {
    stickerData = await writeExifVid(buff, options)
  } else {
    stickerData = await videoToWebp(buff)
  }
